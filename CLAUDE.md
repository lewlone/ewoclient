...
### Phase E — In-game HUD 🟢 E0–E5 shipped (full HUD + overlay input + editor)
...
4. **Draw-direct in the spike; two-clock as of E1.** The spike painted `fbo 0` directly every frame. **E1 (done 2026-05-20)** replaced that with the decoupled model: `paint` renders the HUD to an offscreen GPU surface (rate-gated by `HudPaintRate`), `composite` blits it onto `fbo 0` every frame so it never tears. Verified live with the rate forced to 30 — chunky panel, game still 400–500 fps. The known tradeoff: glass panels no longer backdrop-blur the *live* game (E7 decides if the overlay gets it back). Details in `PHASE_E_PLAN.md`.

E2 shipped the first real widget (a live FPS readout). E3 finished the read-only widget set: **FPS, Coords, Ping, Keystrokes, Armor, PotionHUD, TargetHUD** — all Velvet re-skins of `hud.jsx` elements, in `crates/ewo-jni/src/hud.rs`. The data pipeline is a shared direct `ByteBuffer`: the mod allocates it once (`EwoHudData`), fills it each frame, and Rust reads it through its address (`GetDirectBufferAddress` via the `jni-sys` crate) — `nativeRender()` takes no args, zero per-frame JNI marshaling. A `SCHEMA_VERSION` guards the byte-for-byte layout mirror between `EwoHudData.java` and `hud.rs`. E4 added overlay input: Right Shift opens a custom `Screen` (`EwoOverlayScreen`) that frees the cursor and forwards mouse input to Rust. E5 made the HUD editable — drag widgets, snap-to-align, toggle them, anchor them via a side panel; the layout persists to `<config>/EwoClient/hud.toml`.

**The forward plan for the real HUD is [`PHASE_E_PLAN.md`](PHASE_E_PLAN.md)** — a 7-step build sequence (E1–E7) covering the two-clock paint/composite cache, the JVM→Rust data pipeline, input routing, the `hud.jsx` editor model, the in-game settings overlay, and the Velvet re-skin. The spike proved the pipeline; that file is the plan for building the HUD. **E0–E5 are done; start at E6** (the in-game settings overlay — bundled-mod toggles + HUD prefs).
...
*Last meaningful structural change to this file (2026-05-20 session): Phase E **E5** shipped + verified live — the HUD editor. Widget placement is now data-driven from a persisted `HudLayout` (`<config>/EwoClient/hud.toml`); while the overlay is open, widgets can be dragged (with 6px snap-to-align + guide lines), and a left-edge side panel offers per-widget on/off toggles + a 3×3 anchor preset grid. Earlier this session: **E4** — overlay input: Right Shift opens `EwoOverlayScreen`, a custom `Screen` used as a pure input sink (MC frees the cursor + routes input to it; it renders nothing; the Skia HUD paints the overlay). **E3** — the full read-only HUD (FPS, Coords, Ping, Keystrokes, Armor, PotionHUD, TargetHUD) over a shared direct `ByteBuffer` (Rust reads it via `GetDirectBufferAddress` / `jni-sys`; `SCHEMA_VERSION` guards the layout mirror). **E2** — the first real widget (FPS), a JDK 25 at `%APPDATA%/EwoClient/jdks/temurin-25/`. **E1** — draw-direct → two-clock paint/composite (`paint` to an offscreen surface rate-gated by `HudPaintRate`, `composite` to `fbo 0` every frame); tradeoff: offscreen painting drops the glass panel's live-game backdrop blur (an E7 call). **E0** — the Phase E spike: `crates/ewo-jni` (cdylib JNI bridge, dedicated GL context) + `ingame-mod/` (the `ewo-hud` Fabric mod, plain javac, no Loom). Headline finding: Skia and Minecraft must NOT share a GL context — a dedicated `wglCreateContext` on MC's window is the isolation model. Phase E build sequence + per-step detail lives in `PHASE_E_PLAN.md`; next up is E6 (the in-game settings overlay). Largest remaining gaps unchanged: Indium upstream-blocked at MC 1.21.1, Hyprland verification, formal pixel-parity pass, DPAPI / Credential Vault encryption for `auth.toml` + `EWO_LOADER_TOKEN`.*
