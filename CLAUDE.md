...
*Last meaningful structural change to this file (2026-05-19 session, late): Distant Horizons bundled (default-off, "visuals" category, 30 MB) and GitHub Release infrastructure put in place. Loader-side `9c6377c` adds `distanthorizons` to `BundledMods.BUNDLED_MODS` + the manifest's libraries[] (28 entries total now). Launcher-side `5890d0c` adds DH to `bundled::CATALOG` with `default_on: false` and wires `downloads::job::github_auth_headers_for` — attaches `Authorization: Bearer <EWO_LOADER_TOKEN>` only on URLs matching `api.github.com/repos/<owner>/<repo>/releases/assets/<id>`, strict prefix filter so the token never leaks to Modrinth / Mojang / browser-download URLs. The fat jar is now mirrored to GitHub Releases as `v0.19.2-bundle.1` (asset id 424376159) as an off-box backup; loader manifest's fat-jar URL was briefly switched to the GitHub asset URL and then reverted to `file://` in `21f0fe5` because active dev iteration is still file-based (rebuild + relaunch with no upload step). Launcher auth code stays dormant + ready for the day the GitHub URL gets flipped back. Bundle is now 16 user-toggleable + 5 infrastructure libs. Largest remaining gaps: Indium still upstream-blocked at MC 1.21.1, Hyprland verification, formal pixel-parity pass, DPAPI / Credential Vault encryption for `auth.toml` + the new `EWO_LOADER_TOKEN`, and Phase E (in-game GUI host) which is now unblocked by the bundle being big + stable enough.*
