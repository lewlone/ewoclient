...
### Phase E — In-game HUD 🟢 E0–E3 shipped (spike, two-clock, full read-only HUD)
...
E2 shipped the first real widget (a live FPS readout). E3 finished the read-only widget set: **FPS, Coords, Ping, Keystrokes, Armor, PotionHUD, TargetHUD** — all Velvet re-skins of `hud.jsx` elements, in `crates/ewo-jni/src/hud.rs`. The data pipeline is a shared direct `ByteBuffer`: the mod allocates it once (`EwoHudData`), fills it each frame, and Rust reads it through its address (`GetDirectBufferAddress` via the `jni-sys` crate) — `nativeRender()` takes no args, zero per-frame JNI marshaling. A `SCHEMA_VERSION` guards the byte-for-byte layout mirror between `EwoHudData.java` and `hud.rs`.

**The forward plan for the real HUD is [`PHASE_E_PLAN.md`](PHASE_E_PLAN.md)** — a 7-step build sequence (E1–E7) covering the two-clock paint/composite cache, the JVM→Rust data pipeline, input routing, the `hud.jsx` editor model, the in-game settings overlay, and the Velvet re-skin. The spike proved the pipeline; that file is the plan for building the HUD. **E0–E3 are done; start at E4** (input plumbing + the overlay open/close keybind).
...
*Last meaningful structural change to this file (2026-05-20 session): Phase E **E3** shipped + verified live — the full read-only HUD. `crates/ewo-jni/src/hud.rs` now draws FPS, Coords, Ping, Keystrokes, Armor, PotionHUD and TargetHUD, all Velvet re-skins of `hud.jsx` elements. The JVM→Rust data pipeline is a shared direct `ByteBuffer`: the mod allocates it once (`EwoHudData`), fills it each frame, Rust reads it via `GetDirectBufferAddress` (the `jni-sys` crate) — `nativeRender()` is arg-free, zero per-frame JNI marshaling; a `SCHEMA_VERSION` guards the layout mirror. Earlier this session: **E2** — the first real widget (FPS), a JDK 25 installed at `%APPDATA%/EwoClient/jdks/temurin-25/`, `EwoHudMixin` simplified to a class-literal mixin (the `TracyFrameCapture` stub gone). **E1** — `crates/ewo-jni` went draw-direct → two-clock paint/composite (`paint` to an offscreen surface rate-gated by `HudPaintRate`, `composite` to `fbo 0` every frame); the known tradeoff is offscreen painting drops the glass panel's live-game backdrop blur (an E7 call). **E0** — the Phase E spike: `crates/ewo-jni` (cdylib JNI bridge, dedicated GL context) + `ingame-mod/` (the `ewo-hud` Fabric mod, a `flipFrame` mixin, plain javac, no Loom) + a `file://` entry in `EwoLoaderV1/manifest/0.1.0/26.1.json`. Headline finding: Skia and Minecraft must NOT share a GL context — a dedicated `wglCreateContext` on MC's window is the isolation model. Phase E build sequence + per-step detail lives in `PHASE_E_PLAN.md`; next up is E4 (input plumbing + the overlay open/close keybind). Largest remaining gaps unchanged: Indium upstream-blocked at MC 1.21.1, Hyprland verification, formal pixel-parity pass, DPAPI / Credential Vault encryption for `auth.toml` + `EWO_LOADER_TOKEN`.*
