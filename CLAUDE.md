...
### Phase E — In-game HUD 🟢 E0–E6 shipped (full HUD + editor + settings dashboard)
...
*Last meaningful structural change to this file (2026-05-20 session): Phase E **E6** shipped + verified live — the overlay became a 3-tab dashboard (**HUD · MODS · SETTINGS**). MODS is a Velvet re-skin of a ClickGUI module list — the bundled mods with on/off toggles; the launcher↔overlay write-back (`crates/ewo-launcher/src/overlay_mods.rs`) is a per-instance `overlay-mods.toml` (launcher→overlay snapshot) + `overlay-mod-overrides.toml` (overlay→launcher delta, consumed next launch). SETTINGS holds the `HudPaintRate` cap as a real control, persisted in `hud.toml`. Earlier this session — **E5** the HUD editor (drag + 6px snap-to-align + a side panel with per-widget toggles + a 3×3 anchor grid; layout in `hud.toml`); **E4** overlay input (`EwoOverlayScreen`, a custom `Screen` used as a pure input sink); **E3** the full read-only HUD (FPS/Coords/Ping/Keystrokes/Armor/PotionHUD/TargetHUD) over a shared direct `ByteBuffer` read via `GetDirectBufferAddress`/`jni-sys`; **E2** the first widget (FPS) + a JDK 25 at `%APPDATA%/EwoClient/jdks/temurin-25/`; **E1** draw-direct → two-clock paint/composite (rate-gated by `HudPaintRate`; tradeoff: offscreen painting drops the glass panel's live-game backdrop blur — an E7 call); **E0** the spike — `crates/ewo-jni` (cdylib JNI bridge, dedicated GL context) + `ingame-mod/` (the `ewo-hud` Fabric mod). Headline finding: Skia and Minecraft must NOT share a GL context — a dedicated `wglCreateContext` on MC's window is the isolation model. Phase E build sequence + per-step detail lives in `PHASE_E_PLAN.md`; next up is E7 (polish — glass-refract decision, Velvet re-skin pass, perf). Largest remaining gaps unchanged: Indium upstream-blocked at MC 1.21.1, Hyprland verification, formal pixel-parity pass, DPAPI / Credential Vault encryption for `auth.toml` + `EWO_LOADER_TOKEN`.*
