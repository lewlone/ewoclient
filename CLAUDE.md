...
### Phase E — In-game HUD 🟢 E0–E4 shipped (full read-only HUD + overlay input)
...
E2 shipped the first real widget (a live FPS readout). E3 finished the read-only widget set: **FPS, Coords, Ping, Keystrokes, Armor, PotionHUD, TargetHUD** — all Velvet re-skins of `hud.jsx` elements, in `crates/ewo-jni/src/hud.rs`. The data pipeline is a shared direct `ByteBuffer`: the mod allocates it once (`EwoHudData`), fills it each frame, and Rust reads it through its address (`GetDirectBufferAddress` via the `jni-sys` crate) — `nativeRender()` takes no args, zero per-frame JNI marshaling. A `SCHEMA_VERSION` guards the byte-for-byte layout mirror between `EwoHudData.java` and `hud.rs`. E4 added overlay input: Right Shift opens a custom `Screen` (`EwoOverlayScreen`) that frees the cursor and forwards mouse/keyboard to Rust (`nativeMouse*`/`nativeKey`); while open the game gets no input, while closed the HUD is display-only.

**The forward plan for the real HUD is [`PHASE_E_PLAN.md`](PHASE_E_PLAN.md)** — a 7-step build sequence (E1–E7) covering the two-clock paint/composite cache, the JVM→Rust data pipeline, input routing, the `hud.jsx` editor model, the in-game settings overlay, and the Velvet re-skin. The spike proved the pipeline; that file is the plan for building the HUD. **E0–E4 are done; start at E5** (the draggable HUD editor — drag/anchor/toggle on a scaled stage).
...
*Last meaningful structural change to this file (2026-05-20 session): Phase E **E4** shipped + verified live — overlay input. Right Shift opens `EwoOverlayScreen`, a custom Minecraft `Screen` used purely as an input sink: MC frees the cursor + routes mouse/keyboard to it (game gets nothing), it renders nothing (`extractRenderState` empty), the Skia HUD paints the overlay. `KeyboardHandlerMixin` opens it; four JNI exports (`nativeMouse*`/`nativeKey`) feed a `hud::Input`; a `FLAG_OVERLAY` buffer bit + a placeholder overlay panel prove the path. Earlier this session: **E3** — the full read-only HUD (FPS, Coords, Ping, Keystrokes, Armor, PotionHUD, TargetHUD) over a shared direct `ByteBuffer` (the mod fills it, Rust reads via `GetDirectBufferAddress` / `jni-sys`; `SCHEMA_VERSION` guards the layout mirror). **E2** — the first real widget (FPS), a JDK 25 at `%APPDATA%/EwoClient/jdks/temurin-25/`, `EwoHudMixin` simplified to a class-literal mixin. **E1** — draw-direct → two-clock paint/composite (`paint` to an offscreen surface rate-gated by `HudPaintRate`, `composite` to `fbo 0` every frame); tradeoff: offscreen painting drops the glass panel's live-game backdrop blur (an E7 call). **E0** — the Phase E spike: `crates/ewo-jni` (cdylib JNI bridge, dedicated GL context) + `ingame-mod/` (the `ewo-hud` Fabric mod, plain javac, no Loom). Headline finding: Skia and Minecraft must NOT share a GL context — a dedicated `wglCreateContext` on MC's window is the isolation model. Phase E build sequence + per-step detail lives in `PHASE_E_PLAN.md`; next up is E5 (the draggable HUD editor). Largest remaining gaps unchanged: Indium upstream-blocked at MC 1.21.1, Hyprland verification, formal pixel-parity pass, DPAPI / Credential Vault encryption for `auth.toml` + `EWO_LOADER_TOKEN`.*
