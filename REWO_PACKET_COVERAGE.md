...
1. **141 clientbound-play packets. Rewo resolves and consumes 81 of them. 60
...
2. **The 60 gaps split 6 / 20 / 23 / 11** across pure state, needs-rendering,
...
| Status | Count |
|---|---|
| Resolved **and** consumed | **81** |
| Resolved but ignored | **0** |
| Not resolved at all | **60** |
| **Total clientbound-play** | **141** |

The 60 gaps, by class:

| Class | Count | Share of the gap |
|---|---|---|
| **A** pure state, no rendering | **6** | 10% |
| **B** needs rendering | **20** | 33% |
| **C** needs a subsystem Rewo lacks | **23** | 38% |
| **D** not applicable | **11** | 18% |

M67 audited 56 / 0 / 85 with class A at 31. Sixteen packets separate that
published 56 from M74's 72, and **ten of them had already landed when M67
...
M74's. M75 added `player_abilities`, and M78 the eight of §9.

**Class A is nearly exhausted.** Six pure-state gaps remain, and two of the six
(`player_rotation`, `player_look_at`) are one milestone — §3. The next-largest
tranche of decodable-today work is class **B**, where the decode is possible
and the *feature* wants an eyeball.
