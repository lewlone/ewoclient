...
| `explode` (36) | The physics prefix (M68) **and, since M162, the whole tail**: `read_explode_tail` walks `explosionParticle` through the 22-row option table in `particle_options.rs`, reads `explosionSound`, walks the weighted list, and queues the sound `handleExplosion` plays first — volume 4.0, BLOCKS, pitch `(1 + (nf - nf) * 0.2) * 0.7`, seed `Level.random.nextLong()`. | The weighted list's CONTENTS. They are walked and discarded because they belong to `ClientExplosionTracker`, which is a separate milestone. **The old entry here said the tail needed "M37 kinds Rewo has not transcribed" and ~125 option codecs; both were wrong** — vanilla's only `ExplosionParticleInfo` construction site is `{POOF, SMOKE}` (`Level.java:105-108`), both transcribed since M37, and 103 of the 125 particle types carry zero option bytes. |
| `level_event` (46) | **Both halves, completely as of M162.** The particle half through M37's `route_level_event`; the sound half through `route_level_event_sound` — the block CENTRE (`Level.java:475`), the `data` gates and the per-row volume (M140), plus the three camera-placed ids, the one listener-placed id and the nine distance-delayed ones (M162). | Nothing. **The old entry said the delay is `distance / 340` s; it is `sqrt(distanceToSqr) / 40.0` seconds then `(int)(x * 20.0)` ticks** (`ClientLevel.java:750-751`), gated on `distanceToSqr > 100.0`. 340 appears nowhere in `Level.java`, `ClientLevel.java` or `client/sounds/`; at 100 blocks it gives 5 ticks where vanilla gives 50. It also said "four ids of seventy" — it is three camera plus one listener, of 63 distinct ids. |
...
| `player_info_update` (70) — **closed for the tab list, M151** | Every action the tab list reads: `ADD_PLAYER`, `UPDATE_GAME_MODE`, `UPDATE_LISTED`, `UPDATE_LATENCY`, `UPDATE_LIST_ORDER`, `UPDATE_HAT`, and — since M151 — **`UPDATE_DISPLAY_NAME`**, as `Option<Option<Nbt>>`, because present-with-null *clears* the override and is a third state a `bool` cannot hold. | `INITIALIZE_CHAT` is still walked and discarded. Kept as a worked example: this row said `UPDATE_DISPLAY_NAME` was walked-and-discarded from M62 to M151, and it was the one action of the seven with a **variable-length** payload, so the discard was load-bearing for the walk while the value was simply lost. Servers set custom tab names constantly. |
...
| `login` (49) — **closed, M151** | Everything, now including the two trailing booleans **`onlineMode`** and `enforcesSecureChat`, which sit *after* the embedded spawn info (`ClientboundLoginPacket.java:59-60`) — which is why `read_login_prefix` never reached them and why nothing in the tree read either until M151. | `enforcesSecureChat` is decoded and read by nothing: vanilla uses it only to seed a `SignedMessageValidator` and to decide whether to show the insecure-chat toast. Recorded because this row did not exist before 2026-08-14 — the field was uncaptured, §4 did not list it, and the only mentions in the tree were two doc comments and a *test fixture writing it*. **`onlineMode` is a WIDTH input**, not a visibility one: it is `PlayerTabOverlay`'s `showHead`, so an offline server's rows are nine pixels narrower and every name moves. |
...
| 36 | `explode` | handled | `req!` → `cb_play_explode` | **M68** the physics prefix, **M162** the tail and its sound. §4 partial only for the weighted list's contents, which belong to the explosion-particle tracker. |
...
| 130 | `update_advancements` | handled | `req!` -> `cb_play_update_advancements` | **M177**: reset / added holders / removed ids / progress map / show bool, applied through `rewo_net::advancements::ClientAdvancements` — tree insertion runs parent-before-child PASSES because one packet may list a child before its root; progress is reshaped against the tree node's requirements (prune unnamed criteria, fill missing slots) before storage. `announce_to_chat` never crosses the wire (fromNetwork hard-falses it). The screen that draws it shipped as M178. |
