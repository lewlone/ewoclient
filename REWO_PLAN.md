...
> component model, which Rewo models only as a patch". **Rewo has one** —
> `rewo_data::item_components_table::prototype_has_component`, generated by
> **M56** for the tooltip's component count, covering all 1537 items. M93e uses
> it for the grindstone's two `has()` terms. Check that table before declaring
> any component question unanswerable; the shape to remember is that
> `has(X)` = removed ? false : patch-set ? true : prototype.
