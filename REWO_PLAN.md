...
**The quick-move arc is CLOSED bar one.** M93a–g transcribed **seven of the
eight** single-input menus — merchant, anvil, beacon, stonecutter, grindstone,
cartography table, loom — plus the crafting family in M92e. Nine of the 25
menus now route a shift-click.

| Menu | Blocker |
|---|---|
| `smithing` | `canMoveIntoInputSlots` is overridden with three `RecipePropertySet` tests (`SMITHING_BASE` / `_TEMPLATE` / `_ADDITION`), which arrive on **`update_recipes`** — class C. **The only genuine blocker.** |

Unlike the stonecutter's, these three sets are *not* jar-derivable the way M91's
were: `RecipePropertySet` is built by `create(...)` over a recipe's
**ingredients**, and a smithing recipe's three ingredient slots are per-recipe
rather than one flat `ingredient` field, so the M91 extractor does not reach
them. Worth re-checking against the decompile before assuming it, but it is not
the one-generator job the stonecutter turned out to be.
...
dead. M13's night vision and M19's mining-fatigue swing adjustment had never
once worked live, because five `mob_effect` ids were read from a
`registry_data` branch that cannot fire — and `lightmapshot` and `swingshot`
could not see it, because both *construct* the effect state and supply the very
ids the live path fails to obtain. **When a gate supplies an input that
production must derive, the derivation is untested by construction.** It is
worth a sweep for other instances.

---

*The rest of this section is the 2026-08-02 audit, retained as written.*
...
0. **(M92) A gate that supplies an input production must derive leaves the
   ...
   `RegistryDataLoader.java`'s synchronised list — so M13's night vision and
   darkness and M19's mining-fatigue swing adjustment had never worked live.
   `lightmapshot` and `swingshot` are serverless and *construct* the effect
   state, supplying the very ids the live path fails to obtain, so both proved
   their formulas exactly over a dead input. The rule this restores is already
...
0b. **(M92) A mutation harness that restores by `mv` silently grades the
   ...
   *older* than the mutated build, so cargo considers the artifact current and
   skips the rebuild. It presents as a witness regressing with no change to its
   code. Bump the mtime after restoring. Applies to any timestamp-based build
   system.

0c. **(M92) A sprite NAME is not a file path.** Most GUI sprites resolve under
   ...
   so the sprite `mob_effect/speed` comes from `textures/mob_effect/speed.png`.
   Read the atlas's source list rather than assuming the prefix.
...
the page's recipe slots, the arrows and the filter toggle — and the **menu
displacement**, which is what makes this not a pure addition.

**`updateScreenPosition` is the headline, and the milestone was scoped without
knowing about it.** Opening the book replaces `leftPos = (width - imageWidth) /
2` with `177 + (width - imageWidth - 200) / 2` — 77 px right for a 176-wide
panel. Rewo measures every panel-relative thing from that origin (slot
hit-testing, the icons drawn into slots, the hover box), so drawing the book's
pixels *without* the shift leaves the menu centred under a book that overlaps it
and every click lands on the wrong slot. It is a **silent** failure: the render
looks plausible and the input is wrong. `topPos` does not move. The shifted form
is also not "centre the pair" — it is a literal 177 plus the centring of a
200-wide notional block.

The draw and the hit test now resolve their origin through one `Placement`, so
they cannot disagree — M89's one-accessor rule applied to geometry rather than
to a menu.

**Two design errors, both found by `live --render-check`, neither visible to any
headless gate.**

1. **The book was first hung off `ContainerPanel`** — which `container_panel`
   returns `None` for on the player's own inventory (the path `inventoryshot`
   pins), and the player's inventory is one of exactly **four** screens that has
   a book. So the book was undrawable in its commonest case. `containershot`
   structurally cannot see this: it only ever drives an *open container*. The
   book is now its own field beside the panel, which is what M94a's finding said
   it was in the first place.
2. **The gate's "crafting table" fixture used menu type 13, which is
   `enchantment`.** Same 176x166 size, so every headless witness measured what it
   expected and passed. Only opening it in the windowed client and finding no
   book showed it. `crafting` is 12.

Both are the M86 shape, and both argue the same thing: **run `--render-check` on
any milestone that adds a render path.**

**Four inversions in the chrome**, each mutation-tested:

- **A tab's sprite tracks SELECTION, not hover.** `sprites.get(true,
  this.selected)` hard-codes `enabled` and passes `selected` into the *focused*
  argument, so a tab never responds to the cursor — `handleCursor` is overridden
  to suppress even the pointer while selected. The filter toggle inverts the same
  record the other way: `textures.get(filtering, isHoveredOrFocused())` puts "is
  filtering" in the `enabled` slot, and the sprite *names* (`filter_enabled` /
  `filter_disabled`) make reading it as the widget's own state easy — which gives
  a button that never changes when you click it.
- **A selected tab is drawn 2 px further left and its icon moves with it.**
  Shifting one and not the other reads as a misplaced icon rather than a wrong
  offset. One icon sits at +9 and a pair at +3/+14, and 9 is not the pair's
  midpoint.
- **The "stack of recipes" look is two items at different offsets**: vanilla
  draws a copy at `offset + 1`, then **decrements** `offset` and draws the real
  one — (5, 3). Reading `offset--` as applying to the back copy gives (4, 4), two
  items exactly on top of each other, which renders as one and hides the effect.
- **The panel is sampled from `(1, 1)`**, not the sheet's corner.

**The gate's own rewrite was the instructive part.**

- **b1 passed while measuring nothing it claimed.** It probed the book's centre
  and compared open against shut, reading `[0,0,0]` against `[255,255,255]` —
  and neither number was the book. (73,80) is inside recipe slot 7, near its
  black bottom border; and the shut frame had the *menu* over that screen
  position, because an open book moves the menu. **The control changed along with
  the subject**, which is the one thing a frame diff may not do. It now probes
  bare panel outside the menu in both frames and asserts `(55,55,55)`,
  `recipe_book.png`'s own value there.
- **Every witness names its sprite's value, read out of the PNG** — `tab.png`
  139 against `tab_selected.png` 198, `slot_craftable` 139 against
  `slot_uncraftable` 106. "Different from the backdrop" would pass with every tab
  drawing the same art, which is exactly what `get(true, selected)` invites.
- **A mutation deleting `take(view.shown)` survived the gate** and was killed by
  the model's own test — because the gate's fixture sized its slot vec to
  `shown`, making the guard a no-op. The fixture now passes twenty slots whatever
  the page shows, which is vanilla's own structure (`updateButtonsForPage` keeps
  20 buttons and flips `visible`).

**`rewo-gpu` restates the book's four geometry constants rather than importing
them.** The renderer deliberately does not depend on `rewo-world` — it takes
plain numbers, a boundary the Velvet work records — so the copy is paid for by a
cross-crate test in `rewo-app`, the one crate that sees both.

An existing invariant test caught the new sheet: `MENU_BACKGROUND_TEXTURES` is
asserted to be exactly the container screens' textures, and the book is not a
container screen. Named as one explicit exception rather than by loosening the
comparison, plus a second assertion that the exception names something real, so a
stray entry still fails.

**Measured.** **1979 tests / 0 failures**; `containershot` 76 → **83**;
**`live --render-check` 22 → 23/23, validation ON, 0 errors** (debug build, own
server on a free port, stopped afterwards); `inventoryshot` 152, `itemshot` 75,
`handshot` 34, `mobshot` 246/246; demo PNG `2cc56b4acbfb92cb` byte-identical.
**11 model mutations + 7 render mutations, all 18 killed.**

**Open.** The book draws no **items** — the recipe results in the grid and the
tab icons — which needs the GUI item pass to place icons on the book's own
origin. `hasCraftable` is reported `false` for every collection, so every slot
wears the *uncraftable* chrome: it needs `StackedItemContents`, vanilla's "can I
make this from what I hold" solver, and guessing `true` would be worse because
that is the state that tells a player a recipe is available. Nothing can click
the book, so the selected tab and page are pinned to 0 and hover never reaches
the arrows or the filter — the cursor would have to be converted into book space,
which needs the window size only the renderer has. The search box, the page
counter text, the overlay popup and ghost slots are not drawn.
...
The last transcribable single-input quick-move, closing the arc bar smithing.
Three predicates in a fixed order, each also its slot's `mayPlace`.

**The banner test is a class, and the obvious data stand-in is wrong by exactly
one item.** `getItem() instanceof BannerItem` has no direct data form, and every
banner's prototype carries `minecraft:banner_patterns` — set by the very lambda
that constructs the `BannerItem` — so it looks like a faithful derivation.
**`Items.SHIELD` carries it too**, and a shield is a plausible thing to put in a
loom. `BannerItem` is constructed in exactly **one** place (the 16-colour
`ColorCollection`) and `#minecraft:banners` holds exactly those 16, which the
generator *asserts* rather than assumes. `containershot` `d8` grades it from the
other side: the shield's prototype does carry the component and it is still not
a banner.

**The other two are conjunctions**: `is(#LOOM_DYES) && has(DYE)` and
`is(#LOOM_PATTERNS) && has(PROVIDES_BANNER_PATTERNS)`. The tag half is
unpatchable item identity; the component half is the prototype plus the patch,
and a patch can **remove** what the prototype carries — so each gets its own
removal bit. **Two rather than one shared**, because an item is in at most one
of the two tags and a shared bit would let a removal on the *other* component
falsify the wrong predicate.

**A mutation survived and was shown equivalent rather than fixed.** Dropping
`&& prototype_has(DYE)` changes no answer the jar can produce, because every
item in `#loom_dyes` also carries the component — the exact coincidence the
docs warn about. No vanilla-derived fixture can distinguish the readings, so
`d9` pins the **coincidence** instead of the behaviour: if a version ever tags
an item without the component, the conjunction starts mattering and the witness
fires to say so. The *patch* half is reachable and is witnessed in `rewo-world`.

**Two others survived and were real, both the same shape — one of a symmetric
pair.** The dye removal was witnessed and the pattern one was not; and both
were witnessed on the quick-move path and neither on the plain-click path. **A
witness on one of two mirrored terms leaves the other free to be deleted.** Both
mirrors now exist, and the new plain-click witness covers both slots so it is
not itself half a pair.

**A timeout kill left a mutation on disk.** The battery restores in a `finally`,
which a killed process skips — the run died mid-gate with `live_cmd.rs` holding
the tag-only form. Caught by grepping for the mutation markers before anything
else. **That grep is now the first thing to do after any interrupted battery**,
and it is why the batteries are split so each stays inside the tool's 10-minute
cap.

Gates: **1817 tests**, 0 failures; `containershot` 33 → **36**; `inventoryshot`
152, `itemshot` 75, `handshot` 34, `swingshot` 97, `mobshot` 246/246;
**17 mutations, 16 killed and 1 shown equivalent**; demo PNG
`2cc56b4acbfb92cb`.
...
One commit, and it **inverts the arrangement of every other menu in the set**.
The beacon and stonecutter ask about the *item* in the `quickMoveStack` branch
and let the slot accept anything; the grindstone asks about the *slots*
(`!input.isEmpty() && !additional.isEmpty()`) and puts the item predicate in
`mayPlace`. So `SlotKind::GrindstoneInput` is load-bearing for the
**shift-click**, not merely an ordinary click — it is the only thing that stops
a stick reaching a repair slot, via `moveItemStackTo`'s placement pass, and
when it does the move returns false and vanilla `return`s: a stick shift-clicked
into an empty grindstone moves **nothing**.

**A blocker this plan recorded was half wrong.** M93a said the loom and
cartography table needed "a prototype-component model, which Rewo models only
as a patch". **Rewo has one** — `item_components_table::prototype_has_component`,
generated by **M56** for the tooltip's component count. M93e uses it directly.
`has(X)` on a `PatchedDataComponentMap` is a three-step resolution — *removed →
false, patch-set → true, else prototype* — and that shape is reusable for both
remaining menus.

Three consequences of transcribing `isDamageableItem` exactly
(`has(MAX_DAMAGE) && !has(UNBREAKABLE) && has(DAMAGE)`):

- the two prototype flags are carried **separately** though they co-occur on
  all 1537 vanilla items, because a patch can set `max_damage` without `damage`
  and a collapsed flag hands that to the grindstone;
- `UNBREAKABLE` needs **no** prototype term — no item carries it, verified
  against the table — so the patch bit is the whole answer;
- **one** removal flag is exact rather than two, since removing either damage
  component makes the predicate false.

The second disjunct is what admits an **enchanted book**, which is not
damageable at all — and it must not use `ItemSlot::enchanted`, which is
`has_foil()` (M43: `ENCHANTMENT_GLINT_OVERRIDE` wins both ways) *and*
`isEnchanted()`, reading `minecraft:enchantments` alone while a book's live in
`stored_enchantments`. **The canonical grindstone input is exactly the case that
field misses, twice over.** `may_place` now takes the stack as well as its
props, matching vanilla's `mayPlace(ItemStack)`.

**Three mutations survived the first battery; two were real.** Dropping
`has(DAMAGE)` survived because every fixture set both prototype flags together
— fixed with a patched stack where they diverge, which is the case the
two-field design exists for. And **the decoder's removal flag was untested by
construction**: the only witness constructed an `ItemSlot` with it already set,
M92's finding in new code, now asserted from **bytes** in `rewo-net` with both
negative halves pinned. The third was an **anchor collision, not a survivor** —
the merchant is also a 39-slot menu with the player at 3, so the line matched
twice and the mutation never applied.

Gates: **1799 tests**, 0 failures; `containershot` 31 → **32** (`d5` grades the
production prototype lookup); `inventoryshot` 152, `itemshot` 75, `handshot`
34, `swingshot` 97, `mobshot` 246/246; **16 mutations, 16 killed**; demo PNG
`2cc56b4acbfb92cb`.
...
Eight commits. M91 closed the container arc's furnace half and recorded three
open items; this is all three. `container_set_data` now feeds **every** menu
that sends it, the crafting family's `quickMoveStack` no longer declines, and
the first of the bespoke-widget screens is usable.

#### The three data consumers, and how each inverts against the last

**The brewing stand's data slots are the reverse of the furnace's.**
`getBrewingTicks()` is `get(0)` and `getFuel()` is `get(1)`, where
`AbstractFurnaceMenu` puts its *fuel* at 0. Both menus are five bytes on the
wire and neither says which is which; naming them by analogy swaps a 0..20 fuel
level with a 0..400 tick counter, and the result is a fuel bar pinned full and
bubbles that never move.

**Its brew timer counts DOWN** — the field is the ticks *remaining*, so the
arrow is `28 * (1 - t/400)`, empty at the start and full just before the potion
pops. **Its arrow grows downward while its bubbles grow upward**, one function
apart: the arrow's `dy` is a fixed 16, the bubbles' source and destination `y`
move together so their bottom is pinned at 43 (M91's flame shape), and the fuel
bar is a third direction again. And **the arrow truncates where the furnace
ceils**, so at 399 ticks vanilla shows no arrow where a ceil would show a pixel.
`BUBBLELENGTHS` is a table, not a formula; its last entry is **0**, so one frame
in seven is blank; and `tickCount / 2 % 7` divides first, making each frame a
two-tick hold.

**The enchanting table's costs are only a third of what its rows need.** The
affordability test's three inputs come from three different places: `costs` is
`container_set_data`, `getGoldCount()` is **the count of the stack in menu slot
1** (so it arrives on `container_set_content`), and the XP level and creative
flag are the local player's, from M79 and M75. A client wiring the data packet
alone would grey every row the moment it had no lapis value to check against.
**The lapis requirement is the row INDEX plus one, not the cost.** There are
**three row states, not two** — an empty row draws its background and nothing
else, an unaffordable one draws the same background *plus* its numeral. And
`col` does double duty and is **reassigned before the cost text is drawn**, so a
row's name and its cost are different colours and the cost's does not track the
hover.

**The highlight and the tooltip use different rectangles** and neither is a
slip: the click/highlight test is a bare 108x19, the tooltip's is
`isHovering(.., 108, 17, ..)` whose body bleeds a pixel on every side of a box
declared two rows shorter. They agree over the middle and disagree at the edges.

**The beacon says "absent" with 0 where the enchanting table says -1.**
`encodeEffect(e) = e == null ? 0 : id(e) + 1`. Two adjacent menus, two
conventions, the same signed short on the same packet; carrying either across is
a silent off-by-one where every effect renders as its registry neighbour. It is
`holder`'s `id + 1` scheme (M16/M21/M55's recurring fork) inside a **data slot**,
where every other value in the arc is raw.

**An invisible beacon button moves a visible one.** The secondary column's count
is `BEACON_EFFECTS.get(3).size() + 1`, and the `+ 1` is the upgrade slot — which
is `visible = false` until a primary is chosen, yet is still counted into
`totalWidth`. Dropping it slides regeneration 12 px onto the panel's recess. The
two columns also do not share their y arithmetic (primary steps
`22 + tier * 25`, secondary is a fixed 47), the four states are a **priority
chain** rather than independent flags (`!active` first, so a selected button out
of tier range still paints disabled), and the five payment icons are spaced
21, 22, 23, 22 — vanilla's own literals.

#### The chain, and why one range could not express it

`quick_move_destination` returned ONE destination and `CraftingMenu` writes
`if (!moveItemStackTo(stack, 1, 10, false)) { … }`, where `moveItemStackTo`
returns *whether anything moved*. A single range must either always try the grid
— and never cross-move once it is full — or never try it. The return type is now
the **chain to try in order**, and the caller takes the first entry that moves
something; every other menu returns a chain of one.

**This is the branch that makes a crafting table feel different from your
inventory**: a shift-click there fills the 3x3 grid, because vanilla tries
`1..10` first. `InventoryMenu` has no such branch. And **the two crafting menus
put their result at opposite ends** — `CraftingMenu` slot 0, `CrafterMenu` slot
45, after the player's 36 — so `slot_kind` has to ask the menu's own shape.

A mutation found the crafter's `9..45` bound **unobservable**, and the finding
is kept rather than fixed: the result slot is guarded twice, by the range *and*
by `slot_kind` answering `Result`, so removing either alone changes nothing and
removing both is caught. **That redundancy is vanilla's own** — `CrafterMenu`
passes 45 *and* `NonInteractiveResultSlot.mayPlace` returns false.

#### The first bespoke widget

`ServerboundContainerButtonClickPacket` is two var-ints and is the **whole**
input surface for four screens — an enchanting offer, a loom pattern, a
stonecutter recipe and a crafter toggle are all this packet with a different
index. It carries **no state id**: its sibling `container_click` carries one
because it is a prediction the server grades, and this is not.

**The click gate is not the render gate.** `clickMenuButton` additionally
requires slot 0 to hold something — the render leans on `costs[i]` being 0
without an item, which is true but is an invariant maintained in `slotsChanged`,
a different method — and tests the level against **both** `row + 1` and
`costs[row]`. Vanilla sends the packet only when it returns true, and since Rewo
has no server-side menu to ask, this *is* that gate. Ordering is vanilla's too:
the row loop runs **before** `super.mouseClicked`, and a press failing the gate
is not consumed.

#### The bug the beacon's icons uncovered, which was not the beacon's

Sourcing the six effect ids found that Rewo read `night_vision`, `darkness`,
`haste`, `conduit_power` and `mining_fatigue` from the Configuration
`registry_data` packet, inside a `registry == "minecraft:mob_effect"` branch
**that cannot fire**. `registry_data` carries only
`RegistryDataLoader.SYNCHRONIZED_REGISTRIES`, and `Registries.MOB_EFFECT`
appears **zero times** in that file — a `BuiltInRegistries` entry the server
never sends. All five stayed `None` for the whole session, so **M13's night
vision and darkness and M19's dig-speed / mining-fatigue swing adjustment did
nothing live**.

**No gate could see it, and that is the transferable part.** `lightmapshot` and
`swingshot` are serverless and *construct* the effect state themselves,
supplying the very ids the live path fails to obtain. M86's shape exactly — and
the second time in this project that a serverless oracle proved a formula while
the input feeding it was dead. The rule was already written down in
`attributes.rs` and `sound_events.rs` (a built-in registry is resolved **by name
from the report**) and this module was the only one reading the wire for it. The
regression witness asserts the **source**, not the values: a value witness would
have passed against the broken code too.

#### Three detector errors, one structural duplication, and a harness bug

Extending `containershot` to grade the overlays as pixels took four corrections.

* **o5 compared an empty table against an affordable one.** Those have different
  backgrounds, and the background rect *contains* the numeral rect, so
  suppressing every numeral left it passing — the detector was measuring the
  background. The control is now an *unaffordable* table, which shares the empty
  one's background, and **o5b asserts that it does**.
* **o5's probe was then one row** of the numeral, which is a glyph's transparent
  second row. It reads the whole 16x16 rect.
* **o8 probed the whole upgrade button**, but `beacon_upgrade_effect` reads
  `choice.primary` independently, so the icon appears whether or not the button
  is visible. It probes the 2 px chrome ring.
* **`enchant_row_sprites` had its own copy of the numeral mapping**, so emptying
  `EnchantRow::numeral()` changed nothing rendered — a model accessor graded by
  unit tests the app did not call. M18's and M45's finding in one.

**And the harness bug, which is general.** Restoring a mutated file with `mv`
preserves the ORIGINAL's mtime, which is older than the mutated build, so cargo
considers the mutated binary current and the next run silently grades it. It
showed as a witness regressing with no change to its code — o4 went
green→red→green with nothing between. Any mutation harness against a
timestamp-based build system has this hazard; the restore must bump the mtime.

#### The windowed client, again

`containershot` grades the overlays offscreen and the render-check's injected
container is a **chest**, which has none — so nothing said the windowed client
ever reached the overlay builder. `r22` injects a brewing stand with data at
0.85 and reads the overlay count back **out of the renderer** (asking the menu
answers from the model). It reports **3**; mutation-tested to 0.

#### Measured

**1773 tests** (from 1712), all seven crates confirmed reporting — world
545→600, data 188→192, net 579→581. `containershot` 17 → **27**,
`live --render-check` 21 → **22/22** validation ON and 0 validation errors,
`inventoryshot` 152/152, `itemshot` 75/75, `handshot` 34/34, `hudshot` 41/41,
`mobshot` 246/246, `blockentityshot` 172/172, `swingshot` 97/97, and the rest of
the battery green. Demo PNG `2cc56b4acbfb92cb`, byte-identical.
**41 mutations across the eight commits, 40 killed**; the survivor is the
doubly-guarded crafter bound above, whose two-guard partner is caught.

Note `live --render-check` must be run from a **debug** build: validation is
`cfg!(debug_assertions)`-gated for `live`, so a release binary reports `r17`
false and makes `r18` vacuous.

#### Open

`container_set_data` is now consumed by every menu that sends it. Still
declining in `quickMoveStack`: the **brewing stand** (needs three item
predicates — the `BREWING_FUEL` tag, the four potion items, and
`PotionBrewing.isIngredient`), the **enchantment table** (its last branch is not
a range move at all — it places exactly ONE item into slot 0), the **beacon**,
and the eight item-combiner / single-input menus.

Of the bespoke widgets, the enchanting rows are done. Still open, with the
specific blocker for each: the **beacon** needs `set_beacon` plus the screen's
own click-tracked `primary`/`secondary` (vanilla moves them *before* the server
hears); the **anvil** needs a text field and `rename_item`; the **merchant**
needs `merchant_offers`, which is class C; the **loom** needs its pattern list;
the **stonecutter** needs `update_recipes`, also class C; the **crafter**'s
toggles need its disabled-slot state. The enchanting table's Standard Galactic
name needs a second bitmap font and `EnchantmentNames`' word list, and its clue
tooltip would come from M42's registry.
