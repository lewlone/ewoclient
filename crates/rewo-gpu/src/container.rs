...
impl ContainerPass {
    pub fn new(
        ...
        // One shelf run over both lists, so the small 14x14 / 24x16 overlays
        // fill the gap the 256-wide sheets leave rather than starting a row.
        let (all_sheets, atlas_h) = shelf_pack(
            sprites
                .menu_backgrounds
                .iter()
