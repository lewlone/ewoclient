...
impl Fixture {
    ...
    /// [`Self::send_meta`] with a language table, for the M161 nametag
...
/// M161 — `Entity.DATA_CUSTOM_NAME` is a **component**, and the wire decode
...
fn check_name_flatten(c: &mut Checker, f: &Fixture, baked: &assets::BakedAssets) {
    ...
    c.record(
        ...
        format!(
            "with the jar's table {resolved:?}, without one {unresolved:?} \
             ...
             before M161 drew, because `metadata::parse` called \
    ...
    c.record(
        ...
        format!(
            "{coded:?} — before M161 this was the four characters \
