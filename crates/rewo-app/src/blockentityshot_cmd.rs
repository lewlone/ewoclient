...
const EXPECTED_WITNESSES: usize = 191;
...
fn check_sign_style(
    ...
    // M163's table. Hand-built rather than loaded from the jar because this
    ...
    // ── M163: the sign's lines are components, and the collector flattens them
    ...
    c.record(
        ...
        resolved.len() == 1
            ...
            // pre-M163 client drew for every translatable sign line.
            //
        ...
        format!(
            "with a table {:?}, without one {:?} — the second is the pre-M163 \
    ...
    c.record(
        ...
        format!(
            "{:?} — before M163 this was the four characters {:?}, board width \
...
/// M164 — the carried-water table (`BakedAssets::fluid`) against the raw
...
/// `BakedAssets::water`, which M164 changed.
fn check_carried_fluid_table(
    ...
    // ---- M164b: which SPRITE the carriers sample --------------------------
    //
    // `CarriedFluid::layer` and `raw_layer` were graded by NOTHING when M164
