...
impl LegacyRandom {
    ...
    /// (M161). `crate::particles::LegacyRandom::next_double` is the right one.
    ///
    ...
    ///
    /// Added by M161 for the explosion sound's seed, which is
...
mod tests {
    ...
    //   Rng(0).nextDouble()    == 0.7309677600860596  <- WRONG, see M161: the
    ...
    /// **Renamed by M161: this pins what this module DOES, not what the JVM
    ...
    /// — and finding the fourth is what this test is for (M161).
    ///
    ...
    /// wrong and `crate::particles`' is right** (found by M161, whose own
