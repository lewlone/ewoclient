...
pub fn resolve_for(
    ...
    let title = parse_component(&objective.display_name, base, input.lang);
    let title_width = line_width(&title, input.width_of);
    let spacer_width = (input.width_of)(SPACER_TEXT, base);
    ...
    for (owner, score) in rows {
        ...
        let name = format_name_for_team(team, &owner_name, base, input.lang);
        ...
        let score_line = format_value(format, score.value, base, input.lang);
...
pub fn format_name_for_team(
    ...
    let mut out = parse_component(&params.player_prefix, root, lang);
