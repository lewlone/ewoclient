...
/// `ClientLevel.playSound`'s distance delay, in TICKS (M161).
///
/// ```text
...
/// The sound a `level_event` packet asks for (M140; the tails, M161).
///
...
///
/// M161 finishes it: the three camera-placed ids, the one listener-placed id,
...
mod level_event_sound_tests {
    ...
    /// camera-to-event side (M161; M140 declined all of them).
    ///
    ...
    /// origin, AMBIENT, volume 0.25 (M161).
    ///
