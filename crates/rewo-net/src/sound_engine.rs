...
impl SoundEngine {
    ...
    /// nothing about where it was put (M162), and the camera-placed level
...
/// reaches it", and M140 falsified it without noticing** (M162).
...
impl SoundSystem {
    ...
    pub fn set_music_frequency(&mut self, frequency: crate::music::MusicFrequency) {
        // **The CONSTRUCTOR's path, not the callback's** (M162). This used to
        ...
        for ev in events {
            ...
            match built {
                Ok((instance, ramp)) => {
                    ...
                    // M162 — the second producer of a delayed sound, and the
...
mod tests {
    ...
    // ---- M162: the level event's distance delay -------------------------
    ...
    /// queue, and the tick it comes due on (M162).
    ///
    ...
    /// (M162) — `instance_from_event`'s two arms are one call.
    #[test]
