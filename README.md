...
- **Audio is still the largest single gap, and it is now one step from the decision**: M131 added the sound-instance model, the channel budget and a device *seam* on top of the packets, the 1,968-entry registry and the weighted-variant index — and stopped there. What remains is the device itself: a crate (cpal / rodio / kira) and vanilla's attenuation curve. It is the first Rewo milestone that cannot be verified headlessly end to end, so what the gate asserts has to be settled up front.
