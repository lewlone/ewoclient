...
- **Audio exists and nobody has listened to it yet.** `crates/rewo-audio` carries the quantisation, the buffer library, the mixer, an SPSC command ring and a cpal sink; `rewo-net` carries the listener transform and the music fade. **The listening pass is the outstanding work and it is a human's**: `cargo run -p rewo-audio --example listen`. No gate in this project opens an audio device — an absent, muted, exclusive-mode or unplugged one all look identical from inside the process — so everything a machine can check passes, and that is *not* the same claim. It is also not yet wired into the client: `rewo-app` deliberately does not depend on `rewo-audio`, so the 34 gates do not link an audio stack for a subsystem none of them exercises, which is why `rewo live` is still silent. See [`REWO_AUDIO_PLAN.md`](REWO_AUDIO_PLAN.md).
- **M141** transcribed the ten tickable sound ramps — the per-tick volume/pitch/position curves for bees, elytra, guardians, minecarts, riders, sniffers and the ambient loops — and wired the engine to drive them; **M141d** then fed them their velocity. Nothing constructs one yet; the triggers are all packets Rewo already routes. Two findings, both of the kind that punish a careful reader: `MinecartSoundInstance` shadows `AbstractSoundInstance.pitch`, so the pitch ramp it declares is dead code and the transcription that reads the class on its own gives every minecart ride a twenty-second glissando vanilla does not have; and a remote entity's `getDeltaMovement()` is a *decaying echo of the last motion packet* rather than a velocity, so a bee gliding steadily past you has its buzz fade to silence while it is visibly moving.
