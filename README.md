...
- The **chat decoration** — `boundChatType.decorate`, which turns a message into `<Steve> hi`. Every prerequisite is now met: M125 shipped the translatable resolution underneath it, M126 the styled pipeline that carries the two `/msg` types' GRAY + italic, and the `chat_type` registry is a synchronised datapack registry the configuration phase already delivers. This is scheduling rather than a wall.
- **Clickable chat text.** M126 moved this blocker rather than closing it: there is a `Style` on every span now, but `parse_component` reads colour and the five format flags and drops `ClickEvent` / `HoverEvent`, so there is still nothing to click.
