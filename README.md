...
([`REWO_PACKET_COVERAGE.md`](REWO_PACKET_COVERAGE.md) is at 119 handled / 0 ignored / 22 absent — 11 not-applicable plus 11 needing a subsystem Rewo lacks, so picking work there means choosing a subsystem rather than a packet. Its table is machine-checked by a unit test in `ids.rs`.)
