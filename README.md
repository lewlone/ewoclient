...
- **M141** transcribed the ten tickable sound ramps — the per-tick volume/pitch/position curves for bees, elytra, guardians, minecarts, riders, sniffers and the ambient loops — and wired the engine to drive them. Nothing constructs one yet; the triggers are all packets Rewo already routes, but four of the ten need an entity *velocity* the client does not keep. Its headline is a vanilla bug: `MinecartSoundInstance` shadows `AbstractSoundInstance.pitch`, so the pitch ramp it declares is dead code, and the careful transcription — the one that reads the class on its own — gives every minecart ride a twenty-second glissando vanilla does not have.
