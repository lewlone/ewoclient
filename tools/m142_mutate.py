...
MUTATIONS = [
    ...
    # --- the biome loop's transition (M142d) -------------------------------
    (
        E,
        "the incoming loop is spared the fade-out (its runaway fade uncapped)",
        "        for live in &mut self.live {\n            if let Some(crate::tickable::Ramp::BiomeLoop(b)) = &mut live.ramp {\n                b.fade_out();\n            }\n        }\n        let id = current?;",
        "        let id = current?;\n        for live in &mut self.live {\n            if let Some(crate::tickable::Ramp::BiomeLoop(b)) = &mut live.ramp {\n                if live.instance.identifier != id {\n                    b.fade_out();\n                }\n            }\n        }",
        "KILLED",
    ),
    (
        E,
        "a live loop is REPLACED rather than reused (restarts the sample)",
        "        match existing {\n            Some(live) => {",
        "        match None::<&mut Live> {\n            Some(live) => {",
        "KILLED",
    ),
    (
        E,
        "the reuse matches on identifier alone, ignoring the ramp kind",
    ...
    (
        E,
        "the loop is created at volume 0.0 (never plays)",
    ...
    (
        E,
        "a fresh loop is created NOT fading in (holds at silence forever)",
    ...
    (
        A,
        "the loop transition is suppressed when the new biome has none",
    ...
    (
        A,
        "the additions and the mood are change-gated with the loop",
    ...
    (
        A,
        "a moodless record DRAINS the accumulator instead of freezing it",
        "        if let Some(mood) = &ambient.mood {\n            self.mood.tick(mood, feet, eye_y, light, rng, out);\n        }",
        "        let fallback = AmbientMood::legacy_cave();\n        let mood = ambient.mood.as_ref().unwrap_or(&fallback);\n        self.mood.tick(mood, feet, eye_y, light, rng, out);",
        "KILLED",
    ),
    (
        L,
        "the ambient sample uses a fixed quart instead of the position's",
        ...
        "        let id = self.noise_biome_at_quart(0, 0, 0);",
        "KILLED",
    ),
